          ...
          {isInvitation && orgName ? (
            <CardDescription className="text-sm leading-6 text-[#6B7280] dark:text-slate-400">
              <span className="mt-1 flex items-center gap-1.5">
                ...
                <Badge variant="secondary" className="rounded-full bg-violet-100 font-medium text-violet-800 dark:bg-violet-900/50 dark:text-violet-300">{orgName}</Badge>
              </span>
            </CardDescription>
          ) : (
            <CardDescription className="text-sm leading-6 text-[#6B7280] dark:text-slate-400">{t('description')}</CardDescription>
          )}
        </CardHeader>
          ...
          <form onSubmit={handleSignUp}>
            <div className="flex flex-col gap-6">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  className={cn(
                    'h-12 rounded-2xl border-slate-200 bg-white/90 px-4 shadow-none focus-visible:ring-2 focus-visible:ring-violet-200 dark:border-white/10 dark:bg-white/5 dark:text-white dark:focus-visible:ring-violet-800',
                    isInvitation && 'bg-slate-100 text-slate-500 dark:bg-white/10 dark:text-slate-400',
                  )}
                />
              </div>
              <div className="grid gap-2">
              ...
              <div className="grid gap-2">
                <Label htmlFor="repeat-password" className="text-sm font-medium text-[#1F2937] dark:text-slate-200">{commonT('nav.repeat_password')}</Label>
                <PasswordInput
                  id="repeat-password"
                  required
            ...
            {!isInvitation && (
              <div className="mt-6 text-center text-sm text-[#6B7280] dark:text-slate-400">
                ...
                <Link href="/auth/login" className="font-medium text-violet-700 underline-offset-4 transition-colors hover:text-violet-800 hover:underline dark:text-violet-300 dark:hover:text-violet-200">
                  {t('login')}
                </Link>
              </div>
            )}
