...
export const VARIABLE_GROUPS: Array<{ key: string; variables: VariableDef[] }> = [
    ...
    },
    {
        ...
        variables: [
            ...
            { key: 'BANK_LEGAL_REP_FIRST_NAME',    label: 'Nombre del Representante Legal del Banco' },
            { key: 'BANK_LEGAL_REP_LAST_NAME',     label: 'Apellido del Representante Legal del Banco' },
        ],
    },
    {
        key: 'lawyer',
...
/** Flat set of all static variable keys — used for highlight validation */
export const STATIC_VARIABLE_KEYS: Set<string> = new Set(
    VARIABLE_GROUPS.flatMap((g) => g.variables.map((v) => v.key)),
);
